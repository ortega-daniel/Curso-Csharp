...
namespace LinqExercises
{
    ...
    {
        public static List<Product> ProductList { get; } = new() 
        {
        ...
        public static Product FirstSeafoodElement() 
        {
            var result = ProductList
                .Where(product => product.Category.Equals("Seafood"))
                .First();
        ...
        public static void AveragePricePerCategory() 
        {
            var result = ProductList
                ...
                { 
                    Category = group.Key,
                    AverageProductPrice = Math.Round(group.Select(product => product).Average(product => product.UnitPrice), 2),
                });

            Console.WriteLine("\nAverage Unit Price Per Category:");
            foreach (var group in result)
            {
                Console.WriteLine($"Category: {group.Category}, Avg Product Price: {group.AverageProductPrice}");
            }
        }
        ...
        public static List<Product> ProductsUnder12AscOrder() 
        {
            var result = ProductList
        ...
        public static List<Product> Top3MostExpensiveProducts() 
        {
            var result = ProductList
        ...
        public static List<Product> Top3LeastExpensiveProducts()
        {
            var result = ProductList
                .OrderBy(product => product.UnitPrice)
                .Take(3)
        ...
        public static Product ProductWithLongestName() 
        {
            var result = ProductList
        ...
        public static int GeneralStockSum() 
        {
            var result = ProductList.Sum(product => product.UnitsInStock);
            return result;
        }

        // traer el nombre del producto con el id 80
        public static Product ProductWithId80() 
        {
            return ProductList.Find(product => product.ProductID == 80);
        }

        // listado ordenado por la categoria y luego por el precio
        public static List<Product> ProductsOrderByCategoryThenByPrice() 
        {
            var result = ProductList
        ...
        public static Product ProductWithTheMostWords() 
        {
            var result = ProductList
                .OrderByDescending(product => product.ProductName.Trim().Count(c => c == ' '))
                .First();
        ...
        public static Product ProductWithTheGreatestMultiplication() 
        {
            var result = ProductList
                .OrderByDescending(product => product.UnitPrice * product.UnitsInStock)
                .First();
